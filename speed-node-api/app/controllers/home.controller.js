...
const HomeOfferImage9 = db.HomeOfferImage9;
...
exports.createOfferImage9 = async (req, res) => {
    try {
        let get_offer_image8 = await HomeOfferImage9.findOne({});

            if (get_offer_image8) {
                await HomeOfferImage9.findOneAndUpdate({ _id: get_offer_image8._id }, {
                    title: req.body.title,
                    isactive: req.body.isactive,
                    image: req.body.image != null ? req.body.image : ''
                }).then(data => {
                    res.status(200).send({
                        "message": "Footer Detail Updated",
                        "data": data
                    })
                }).catch(ex => {
                    res.status(500).send({
                ...
                const offer_image8 = new HomeOfferImage9({
                    title: req.body.title,
                    isactive: req.body.isactive,
                    image: req.body.image != null ? req.body.image : ''
                })
...
exports.updateOfferImage9 = async (req, res) => {
    // Validate request
    await HomeOfferImage9.findOneAndUpdate({ _id: req.params.id }, {
        title: req.body.title,
        isactive: req.body.isactive,
        image: req.body.image != null ? req.body.image : ''
    }).then(data => {
...
exports.getSingleOfferImage9 = async (req, res) => {
    try {
        await HomeOfferImage9.findOne({}).then(data => {
            if (!data) {
                res.status(404).send({
                    success: false,
                    "message": "There is no Footer Iamge US, Please add your first banner."
                })
            } else {
                res.status(200).send({
                    success: true,
