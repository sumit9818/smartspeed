...
exports.createSubscription = async (req, res) => {
	try {
		...
		if (!req.body.package_id) {
			...
			let membershipType;
			let pid;
			if(req.body.plan_id === 'onetime'){
				package = await Pricing2.findOne({ _id: req.body.package_id });
				membershipType = 'lifetime';
			}else{
				package = await Pricing.findOne({ _id: req.body.package_id });
				...
				payment_subscription
					...
					.then((data) => {
						User.findByIdAndUpdate(req.body.user_id, {
							...
							package_id: req.body.package_id,
							transactions: JSON.stringify(req.body.data)
						}).then((res) => {
...
exports.getAllPayments2 = async (req, res) => {
	await PaymentSubscription.find()
		.sort({ date: -1 })
		.populate("user_id")
		.populate("package_id2")
		.exec((err, data) => {
			if (err) {
				res.status(500).send({ message: err });
				return;
...
exports.getSinglePaymentDetails = async (req, res) => {
	...
	const config = {
		method: "get",
		url: `https://api-m.paypal.com/v1/billing/subscriptions/${req.params.id}`,
		headers: {
...
exports.getAllPaymentDetails = async (req, res) => {
	...
	const config = {
		method: "get",
		url: `https://api-m.paypal.com/v1/billing/subscriptions/${req.query.subscription_id}/transactions?start_time=${req.query.start_time}&end_time=${req.query.end_time}`,
		headers: {
			accept: "application/json",
			"accept-language": "en_US",
			"content-type": "application/json; charset=utf8",
