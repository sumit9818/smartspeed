      ...
      .exec((err, user) => {
        ...
        res.status(200).json({
          data: {
            ...
            gender: user.gender != null ? user.gender : '',
            contact: user.contact,
            membership: user.membership ? user.membership : '',
            transactions:user.transactions ? user.transactions : '',
            package_id:user.package_id ? user.package_id : '',
            level:user.level!= null?user.level:"",
            aimlevel:user.aimlevel!= null?user.aimlevel:"",
             best_suits:user.best_suits!= null?user.best_suits:""
          }
...
exports.userSubscriptionDetails = async (req, res) => {
  ...
  const pricing = await Pricing.findOne({ _id: req.subscriptionId })

  if(user.subscription_id && user.membership != 'lifetime'){
    const config = {
      method: "get",
      url: `https://api-m.paypal.com/v1/billing/subscriptions/${user.subscription_id}`,
      headers: {
