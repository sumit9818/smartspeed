...
export class ImageOneComponent implements OnInit {
	...
	private updateBannerValue(): void {
		this.HomePageService.getOffer1().subscribe(
			data => {this.offer1 = data
			this.picture = this.offer1.data.image
			this.f.isactive.setValue(this.offer1.data.isactive);
			this.f.image.setValue(this.offer1.data.image);
			this.f.title.setValue(this.offer1.data.title);
			}
		)
	}
	...
	private createOffer1() {
		this.HomePageService.AddOffer1(this.form.value).pipe(first()).subscribe(
		data => {
